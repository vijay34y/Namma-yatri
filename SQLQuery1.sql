...
-- total earnings
  select * from trips;
  select sum(fare) Total_drivers_earnings from trips;
...
--total searches which got estimate
 select sum(searches_got_estimate) Total_searches_got_estimate from trips_details4;
...
select sum(searches_got_quotes) Total_searches_got_quotes from trips_details4;
...
select count(*)-sum(driver_not_cancelled) driver_not_cancelled from trips_details4;
...
select count(*)-sum(customer_not_cancelled) customer_not_cancelled from trips_details4;
...
select top 1 faremethod,count(distinct tripid) cont  from trips
group by faremethod
order by count(distinct tripid) desc) b on a.id=b.faremethod;
...
select *,dense_rank() over (order by moretrips desc) rnk from
(select duration,count(distinct tripid) moretrips from trips
 group by duration)a
;
...
group by driverid,custid)a)b where rnk=1
;
...
select *,rank() over (partition by duration order by highest_trips desc) rnk from(
select loc_from,duration,count(distinct tripid) highest_trips from trips
group by loc_from,duration )a
;
